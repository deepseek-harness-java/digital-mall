...
public class DigitalMallAssistantPlugin extends AbstractHarnessPlugin {
    private static final String PLUGIN_ID = "digital-mall-assistant";
    ...
    public DigitalMallAssistantPlugin() {
        super(PLUGIN_ID);
    }
    ...
    public void configure(PluginContext context) {
        ...
        context.registerSystemPrompt("mall-capabilities", 10, """
                ...
                You are the virtual shopping assistant for Digital Mall.
                ...
                2. For ANY product question, recommendation, category request, price, stock, availability, comparison, or "what should I buy" request, including generic device/gift ideas and short requests such as "推荐个数码产品" and "耳机类", you MUST call `search_products` in the next action before writing any recommendation. This rule still applies when the request looks conversational.
                ...
                5. NEVER recommend, mention, or invent products from external merchants, brands, stores, or websites. Do not use any brand or product not returned by this mall's tools.
        ...
        context.registerHook("POST_TOOL_USE", (toolName, payloadJson) -> {
            ...
            context.emit("digital-mall-assistant.tool.used", java.util.Map.of(
                        "tool", toolName,
        ...
        context.registerDisposer(() -> context.emit("digital-mall-assistant.plugin.closed", "plugin shutdown"));
    }
}
