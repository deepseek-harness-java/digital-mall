...
async function loadProducts() {
  $('#productGrid').innerHTML = '<div class="loading">Digital Mall 正在加载商品…</div>';
...
function renderProducts() {
  if (!state.products.length) {
    $('#productGrid').innerHTML = '<div class="loading">没有找到匹配的数码好物，换个关键词试试。</div>';
    return;
  }
  $('#productGrid').innerHTML = state.products.map(product => `
    <article class="product">
      ...
      <div class="product-body">
        ...
        <div class="policy-badge">${esc(product.companyName)} · ${esc(product.workPolicy)}</div>
...
function renderCart() {
  ...
  $('#cartItems').innerHTML = items.length ? items.map(item => `
    ...
    </div>`).join('') : '<div class="empty-state">购物车还是空的<br><br>先挑一件心仪的数码产品吧。</div>';
}
...
function showProductDetail(product) {
  ...
  openModal(`
    ...
    <div class="kv">
      <div><strong>品牌渠道：</strong>${esc(product.companyName)}</div>
      <div><strong>选购建议：</strong>${esc(product.weekendTip)}</div>
      <div><strong>服务保障：</strong>${esc(product.workPolicy)}</div>
  ...
  openModal(`
    ...
    <form id="checkoutForm">
      <div class="form-grid">
        ...
        <div class="field wide"><label>收货地址</label><input name="address" value="上海市徐汇区科技园区数字座 101" required></div>
        <div class="field wide"><label>备注（可选）</label><textarea name="remark">工作日送，不用太着急。</textarea></div>
      </div>
...
async function loadOrders() {
  ...
  if (!orders.length) {
    openModal('<h3>我的订单</h3><div class="empty-state">还没有订单，先把心仪的数码产品放进购物车吧。</div>');
    return;
  }
...
function renderMallCard(body, info) {
  ...
  if (info.kind === 'products') {
    html = `
      <div class="mall-card">
        <div class="mall-card-title"><span>商城商品</span><span>品牌渠道</span></div>
