...
public class AuthenticationFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain)
        ...
        if (publicApi || customer != null || serviceAuthorized) {
            ...
            if (serviceAuthorized && customer == null) {
                request.setAttribute("currentCustomer", new Customer(
                        request.getParameter("customerId"), request.getParameter("customerId"), "DSH Service Client"));
            }
            chain.doFilter(request, response);
            return;
        }
